function tt=canny(track)
global rowM colM
channel_3=imread(track);%打开RGB图片
image_gray=rgb2gray(channel_3); %RGB格式图片转化为灰度图片
image_grayadapthisteq=adapthisteq(image_gray);%adapthisteq均衡化
[row,col]=size(image_grayadapthisteq);
figure;
subplot(1,2,1)%画出子图
imshow(image_gray)
title('灰度图像')
subplot(1,2,2)
imshow(image_grayadapthisteq)
title('直方图均衡化后的灰度图像')
rowM=row;
colM=col;
...
new_gray_channel=double(image_grayadapthisteq)/255;%生成新的灰度矩阵
