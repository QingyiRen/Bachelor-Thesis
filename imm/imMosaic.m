function [imgout,matchLoc1,matchLoc2,trackinglist] = imMosaic( img1,img2,adjColor )
...
for i=1:size(corrPtIdx,2)
    x1ctrl = matchLoc1(corrPtIdx(1,i),1);
	y1ctrl = matchLoc1(corrPtIdx(1,i),2);
	x2ctrl = matchLoc2(corrPtIdx(1,i),1);
	y2ctrl = matchLoc2(corrPtIdx(1,i),2);
    trackinglist(1,i)=round((x1ctrl-1)*n/2+y1ctrl);
    trackinglist(2,i)=round((x2ctrl-1)*n/2+y2ctrl);
end
end
